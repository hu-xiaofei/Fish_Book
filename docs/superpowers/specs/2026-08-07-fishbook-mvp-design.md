...
鱼类资料包含：中文名、别名、学名、封面图片、水域类型、外形特征、栖息环境、地理分布、食性、常见钓法、注意事项、来源链接、草稿/已发布状态和发布时间。草稿允许内容不完整；发布前必须具备中文名、学名、封面、水域类型、外形、栖息环境、分布、食性、钓法和至少一个来源链接。
...
- `fish_aliases(fish_id, alias)` 唯一，避免同一鱼类重复别名。
...
API 使用 `/api/v1` 前缀。普通请求和响应使用 JSON；包含图片的鱼类或钓获创建/修改请求使用 `multipart/form-data`，其中 `metadata` 部分是 JSON，`image` 部分是文件。所有修改请求无论内容类型都必须携带 CSRF Token。主要资源：
