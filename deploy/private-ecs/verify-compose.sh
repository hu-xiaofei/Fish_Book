...
check private_photo_mount '.services.backend.volumes | length == 1 and any(.type == "bind" and .source == "/opt/fishbook/data/photos" and .target == "/data/photos" and (.read_only // false) == false and .bind.create_host_path == false)'
check readonly_tls_mount '.services.frontend.volumes | length == 1 and any(.type == "bind" and .source == "/opt/fishbook/tls" and .target == "/etc/fishbook/tls" and .read_only == true and .bind.create_host_path == false)'
