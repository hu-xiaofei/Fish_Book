...
check admin_bootstrap_mapping '.services.backend.environment as $environment | $environment.FISHBOOK_ADMIN_BOOTSTRAP_ENABLED == "true" and ($environment.FISHBOOK_ADMIN_BOOTSTRAP_EMAIL | type == "string" and length > 0) and ($environment.FISHBOOK_ADMIN_BOOTSTRAP_PASSWORD | type == "string" and length > 0) and ($environment.FISHBOOK_ADMIN_BOOTSTRAP_NICKNAME | type == "string" and length > 0) and ($environment | has("FISHBOOK_ADMIN_EMAIL") | not) and ($environment | has("FISHBOOK_ADMIN_PASSWORD") | not) and ($environment | has("FISHBOOK_ADMIN_NICKNAME") | not)'
